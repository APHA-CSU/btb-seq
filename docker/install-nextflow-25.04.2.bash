...
if [ -f "$JAVA_KEY" ]; then
  ...
  if [[ ! $JAVA_VER =~ ^(1.8|9|10|11|12) ]]; then
      echo_yellow "NOTE: Nextflow is not tested with Java $JAVA_VER -- It's recommended the use of version 8 up to 12\n"
...
if [ -s "$LAUNCH_FILE" ] && [ "$LAUNCH_FILE" -nt "$NXF_BIN" ]; then
    ...
    if [[ "$JAVA_VER" =~ ^(9|10|11|12|13|14) ]]; then
      launcher=("${cli[@]:0:1}")
