...
patternsDetailsFile = "CSSnewclusters_LT708304_230119.csv"
...
def comparePatterns(refPat, strPat, groPat):
    ...
    counts = [round(100*float(counts['M'])/lenPat, 2), round(100*float(counts['MM'])/lenPat, 2),
              round(100*float(counts['N'])/lenPat, 2), round(100*float(counts['A'])/lenPat, 2)]
    return [counts, res]


# this is the key part that runs the per sample stage1 genotyping

def findGenotypeOneSample(strainsDetailsTittle, strainDetails, pathTBRuns, patternsDetails, patternsBritishBTBDetails,
                          patternsBTBDetails, patternsMic_PinDetails, patternsMicrotiDetails, patternsPinnipediiDetails,
                          refName, qth, pathAux, thMinGoodCov, thCovProp, thqualsnp, thqualnonsnp):
...
def getBestMatchPattern(patternsDetails, strainGSSInfo):
    ...
    if hal is False:
        sys.exit("matching positions is false")
